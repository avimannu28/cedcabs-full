...
class request
{
 ...
 {
  $a   = [];
  $sql = mysqli_query($this->conn, "select * from tbl_ride where statuss='1'");
  if (mysqli_num_rows($sql) > 0) {
   while ($row = mysqli_fetch_assoc($sql)) {
    array_push($a, $row);
   }
 ...
 {
  $sql = mysqli_query($this->conn, "UPDATE tbl_ride SET statuss='2' WHERE ride_id=$id");
 ...
 public function cancel_ridereq($id){
      $sql=mysqli_query($this->conn,"UPDATE tbl_ride SET statuss='0' WHERE ride_id=$id");
      return 1;
 }
}
