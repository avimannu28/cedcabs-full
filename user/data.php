...
if (isset($_POST["current"])) {
 ...
 $pickup               = $location->pickup($current);
 $drop                 = $location->drop($destination);
 $cab_distance         = abs($pickup - $drop);
 ...
 if ($luggages <= 10 && $luggages > 0) {
  $total = 50;
 } elseif ($luggages <= 20 && $luggages > 10) {
  $total = 100;
 } elseif ($luggages > 20) {
  $total = 200;
 }
 ...
 if ($cab == "CedMicro") {
  ...
  if ($cab_distance > 0 && $cab_distance <= 10) {
   ...
   echo ($total);
  } elseif ($cab_distance > 10 && $cab_distance <= 60) {
   $first = $cab_distance - 10;
   $total += abs(($first * 12)) + (10 * 13.50);
   echo ($total);
  } elseif ($cab_distance > 60 && $cab_distance <= 160) {
   $first   = 0;
   $secound = 0;
   $first   = $cab_distance - 10;
   $secound = $first - 50;
